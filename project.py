...
def analyze_stationarity(timeseries, title,date_time):
    ...
    ax[0].set_title('1-day window')
    ax[0].legend()
    
    rolmean = pd.Series(timeseries).rolling(window=24*30).mean() 
    rolstd = pd.Series(timeseries).rolling(window=24*30).std()
...
analyze_stationarity(df['dam'], 'dam price over time',date_time)
